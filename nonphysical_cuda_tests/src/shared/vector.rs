...
mod vector_tests {
    ...
    fn ln_ref() {
        Runtime::init(0, "../nonphysical_ptx.ptx");
        let mut data_t = (1..1024 * 1024 + 1)
            .map(|i| F32::usize(i))
            .collect::<Vec<_>>();
        let mut data_k = (1..1024 * 1024 + 1)
            .map(|i| F32::usize(i))
            .collect::<Vec<_>>();
    ...
    fn exp_ref() {
        Runtime::init(0, "../nonphysical_ptx.ptx");
        let mut data_t = (0..1024 * 1024)
            .map(|i| F32::usize(i).sin())
            .collect::<Vec<_>>();
        let mut data_k = (0..1024 * 1024)
            .map(|i| F32::usize(i).sin())
            .collect::<Vec<_>>();
    ...
    fn fma_vec_direct() {
        ...
        let data = (0..1024 * 1024).map(|i| F32::usize(i)).collect::<Vec<_>>();
        let reference = (0..1024 * 1024).map(|i| F32::usize(i)).collect::<Vec<_>>();
        let vec_mul = (0..1024 * 1024)
            .map(|i| F32::usize(i).cos())
            .collect::<Vec<_>>();
        let ref_vec_mul = (0..1024 * 1024)
            .map(|i| F32::usize(i).cos())
            .collect::<Vec<_>>();

        let vec_add = (0..1024 * 1024)
            .map(|i| F32::usize(i).sin())
            .collect::<Vec<_>>();
        let ref_vec_add = (0..1024 * 1024)
            .map(|i| F32::usize(i).sin())
            .collect::<Vec<_>>();

        let out =
            CudaVector::fma_vec_direct(data.into_iter(), vec_mul.into_iter(), vec_add.into_iter());
        let known = FloatVector::fma_vec_direct(
            reference.into_iter(),
            ref_vec_mul.into_iter(),
            ref_vec_add.into_iter(),
        );
        out.zip(known).for_each(|(o, k)| {
            assert!((o - k).l2_norm() < F32::EPSILON);
        }); */
