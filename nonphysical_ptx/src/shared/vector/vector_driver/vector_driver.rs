...
impl<F: Float> CudaVectorHost<F> {
    ...
    pub fn reduce_alloc_prim(host_data: &[F], out: F::Primitive)->VectorArgumentsReducePrim<'_, F>{
        let mut global_data = CuGlobalSlice::alloc(host_data);
        let mut global_acc = CuGlobalSliceRef::alloc(&[out]);
        global_data.store(host_data);
        global_acc.store(&[out]);

        VectorArgumentsReducePrim {
            data: global_data,
            acc: global_acc,
        }
    }
    ...
    pub fn l1_min(runtime: Arc<Runtime>, host_data: &[F]) -> F::Primitive {
        let mut out = [F::Primitive::MAX];
        let mut global_memory = Self::reduce_alloc_prim(host_data, F::Primitive::MAX);

        CudaVectorDevice::l1_min(runtime.clone(), &mut global_memory);
...
impl<F: Float> CudaVectorDevice<F> {
    ...
    pub fn l1_min(runtime: Arc<Runtime>, args: &mut VectorArgumentsReducePrim<F>) {
        let kernel = format!("vector_l1_min_{}", F::type_id());
        Self::vector_launch(runtime, args, args.data.ptr.len(), kernel);
    }
