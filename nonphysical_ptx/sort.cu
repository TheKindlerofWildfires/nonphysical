...
__global__ void mergeSmallBatch_k(int * A, int * sizeA, int *B, int *sizeB, int *M, const int d, int N){
	...
	//d=1
	const int tidx = threadIdx.x % d;//0				// Numéro de la diagonal dans le tableau numéro Qt
	const int Qt = (threadIdx.x - tidx) / d;//thread id			// Numéro du tableau par rapport au tableau shared
	const int gbx = Qt + blockIdx.x * (blockDim.x / d);//thread id		// Numéro du tableau par rapport au tableau global

	const int sizeAi = sizeA[gbx];//8				// Taille du tableau considéré
	const int sizeBi = sizeB[gbx];//8				// Taille du tableau considéré
	...
	Atemp[Qt * d + tidx] = A[gbx * d + tidx];//at[thread_id]=A[thread_id]		// Remplissage du tableau
	Btemp[Qt * d + tidx] = B[gbx * d + tidx];//at[thread_id]=A[thread_id]		// Remplissage du tableau
