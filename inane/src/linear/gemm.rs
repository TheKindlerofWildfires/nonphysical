...
mod gemm_test {
    ...
    use baseless::{linear::gemm::Gemm, random::pcg::PermutedCongruentialGenerator, shared::{complex::{Complex, ComplexScaler}, float::Float, matrix::Matrix}};

    #[test]
    fn naive_static() {
        let m33 = Matrix::new(
            3,
            (0..9).map(|c| ComplexScaler::new(c as f32, 0.0)).collect(),
        );
        let m34 = Matrix::new(
            3,
            (0..12)
                .map(|c| ComplexScaler::new(c as f32, 0.0))
                .collect(),
        );
        let m43 = Matrix::new(
            4,
    ...
    fn gemm_lesser() {
        ...
        let data =
            (0..n1 * n2).map(|_| ComplexScaler::new(pcg.next_u32() as f32 / u32::MAX as f32, 0.0));
