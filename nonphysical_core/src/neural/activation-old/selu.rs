...
pub struct Selu<F: Float> {
    parameters: SeluParameters<F>,
}

pub struct SeluParameters<F: Float> {
    phantom_data: PhantomData<F>,
}

impl<F: Float> Activation<F> for Selu<F> {}

impl<F: Float> Layer<F> for Selu<F> {
    type Matrix = MatrixHeap<F>;
    type Parameters = SeluParameters<F>;
