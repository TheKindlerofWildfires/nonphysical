...
pub struct Mish<F: Float> {
    parameters: MishParameters<F>,
}

pub struct MishParameters<F: Float> {
    phantom_data: PhantomData<F>,
}

impl<F: Float> Activation<F> for Mish<F> {}

impl<F: Float> Layer<F> for Mish<F> {
    type Matrix = MatrixHeap<F>;
    type Parameters = MishParameters<F>;
