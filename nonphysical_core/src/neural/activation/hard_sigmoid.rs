...
pub struct HardSigmoid<F: Float> {
    parameters: HardSigmoidParameters<F>,
}

pub struct HardSigmoidParameters<F: Float> {
    phantom_data: PhantomData<F>,
}

impl<F: Float> Activation<F> for HardSigmoid<F> {}

impl<F: Float> Layer<F> for HardSigmoid<F> {
    type Matrix = MatrixHeap<F>;
    type Parameters = HardSigmoidParameters<F>;
