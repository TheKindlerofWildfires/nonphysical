...
pub struct MaxLayerReduceTotal {
    name: String,
}

impl<F: Float> Layer<F> for MaxLayerReduceTotal {
    ...
    fn forward(&self, x: &Self::Matrix) -> Self::Matrix {
        Self::Matrix::new((1, vec![FloatVector::max(x.data())]))
    }
    ...
    fn backward(
        ...
        let distribution = FloatVector::max(gradient.data());
        let output =  FloatVector::max(memory.data());
    ...
    fn forward_ref(&self, x: &mut Self::Matrix) {
        *x = Self::Matrix::new((1, vec![FloatVector::max(x.data())]))
    }
...
pub struct MaxLayerReduceAxis {
    name: String,
    axis: usize,
}

impl<F: Float> Layer<F> for MaxLayerReduceAxis {
    type Matrix = MatrixHeap<F>;
    ...
    fn backward(
        ...
        if self.axis == 0{
            ...
            memory.data_rows().zip(out.data_rows_ref()).zip(gradient.data()).for_each(|((memory_row, out_row),distribution)|{
                let output = FloatVector::max(memory_row.iter());
            ...
            (0..memory.cols).zip(gradient.data()).for_each(|(i,distribution)|{
                let output = FloatVector::max(memory.data_col(i));
