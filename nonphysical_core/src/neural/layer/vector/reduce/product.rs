...
pub struct ProductLayerTotal {
    name: String,
}

impl<F: Float> Layer<F> for ProductLayerTotal {
    ...
    fn forward(&self, x: &Self::Matrix) -> Self::Matrix {
        Self::Matrix::new((1, vec![FloatVector::product(x.data())]))
    }
    ...
    fn backward(
        ...
        //grad/out * xi *lambda
        let distribution = FloatVector::product(gradient.data());
        let output =  FloatVector::product(memory.data());
    ...
    fn forward_ref(&self, x: &mut Self::Matrix) {
        *x = Self::Matrix::new((1, vec![FloatVector::product(x.data())]))
    }
...
pub struct ProductLayerAxis {
    name: String,
    axis: usize,
}

impl<F: Float> Layer<F> for ProductLayerAxis {
    type Matrix = MatrixHeap<F>;
    ...
    fn forward(&self, x: &Self::Matrix) -> Self::Matrix {
        ...
        if self.axis == 0{
            let data = x.data_rows().map(|row|{
                FloatVector::product(row.iter())
            });
            Self::Matrix::new((1, data.collect()))
        }else{
            let data = (0..x.cols).map(|i|{
                FloatVector::product(x.data_col(i))
            });
    ...
    fn backward(
        ...
        if self.axis == 0{
            ...
            memory.data_rows().zip(out.data_rows_ref()).zip(gradient.data()).for_each(|((memory_row, out_row),distribution)|{
                let output = FloatVector::product(memory_row.iter());
            ...
            (0..memory.cols).zip(gradient.data()).for_each(|(i,distribution)|{
                let output = FloatVector::product(memory.data_col(i));
