...
impl<F: Float> Layer<F> for CosLayer {
    ...
    fn forward(&self, x: &Self::Matrix) -> Self::Matrix {
        let mut out = x.clone();
        FloatVector::cos_ref(out.data_ref());
        out
    }
    ...
    fn backward(
        ...
        //grad * cos(mem)
        let delta = FloatVector::sin(memory.data());
        let delta = FloatVector::neg_direct(delta);
    ...
    fn forward_ref(&self, x: &mut Self::Matrix) {
        FloatVector::cos_ref(x.data_ref());
