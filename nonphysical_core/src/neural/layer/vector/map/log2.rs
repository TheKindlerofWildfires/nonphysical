...
pub struct Log2Layer {
    name: String,
}

impl<F: Float> Layer<F> for Log2Layer {
    ...
    fn forward(&self, x: &Self::Matrix) -> Self::Matrix {
        let mut out = x.clone();
        FloatVector::log2_ref(out.data_ref());
        out
    }
    ...
    fn backward(
        ...
        let delta = FloatVector::mul(memory.data(),(F::IDENTITY+F::IDENTITY).ln());
    ...
    fn forward_ref(&self, x: &mut Self::Matrix) {
        FloatVector::log2_ref(x.data_ref());
