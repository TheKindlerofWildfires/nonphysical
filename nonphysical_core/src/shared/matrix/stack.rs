...
impl<F: Float,const N: usize> Matrix<F> for MatrixStack<F,N> {
    ...
    #[inline(always)]
    fn data_rows<'a>(&'a self) -> impl Iterator<Item = &[F]>  where F: 'a{
    ...
    fn data_north_west<'a>(
        &'a self,
        north: usize,
        west: usize,
    ) -> impl Iterator<Item = &'a F>  where F: 'a{
        self.data_north(north)
            .enumerate()
            .filter(move |(i, _)| i % self.cols < west)
            .map(|(_, c)| c)
    }
    fn data_north_east<'a>(
        &'a self,
        north: usize,
        east: usize,
    ) -> impl Iterator<Item = &'a F>  where F: 'a{
    ...
    fn covariance<'a>(&'a self) -> Self {
        ...
        MatrixStack::new(self.rows, data)
    }*/
}
